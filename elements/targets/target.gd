...
class_name Target
# укажем позицию ножа она будет на ростоянии 180 пикселей
...
var speed := PI
#укажем расположения контейнера для ножей
...
func get_free_random_ratation(occupied_rotations: Array, generation_attempts = 0): #передаём массив и кол_во попыток
	#остановим процес при избытке попыток
	if generation_attempts >= GENERATION_LIMIT:
		return null
	#создадим случайное значение созданной ранее функцией из глобал
...
#проверям, если случайное число между растоянием существующего объекта то запускаем функцию снова иначе выводим координаты
		if random_rotation <= occupied + OBJECT_MARGIN / 2.0 and random_rotation >= occupied - OBJECT_MARGIN / 2.0:
			#повторяем поиск свободного места
